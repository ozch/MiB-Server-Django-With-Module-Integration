...
class TopologyMapper():
        ...
        for bridge in bridges:
            ...
            for key in dict_sw:
                ...
                if self.areInSameNetwork(bridge[0],dict_sw[key]["ip"],host_bits[0]):
                    self.cursor.execute("select * from (select parent_ip,mac,type,port from mib.topology_hardware where parent_ip!='{}' group by port having count(*)>=1) as interface where interface.mac like '{}%'".format(bridge[0],bridge[1][0:11]))
    ...
    def getSwitchTopology(self):
        ...
        for switch in switchs:
            ...
            self.cursor.execute("SELECT mac,port FROM mib.topology_hardware where parent_ip='{}' group by port having count(*)<=1".format(switch[0]))
            macs = self.cursor.fetchall()
            for mac in macs:
                self.cursor.execute("SELECT child_ip,parent_ip FROM mib.topology_devices where hard_address='{}' and parent_type='Switch' and parent_ip='{}'".format(mac[0],switch[0]))
                devices = self.cursor.fetchall()
                for device in devices:
                    if self.areInSameNetwork(switch[0],device[0],switch[1]) and self.isNotASwithcIP(device[0],switchs):
                        temp = {"type":"device","mac":str(mac[0]),"speed":"100","ip":device[0],"interface":mac[1]}
    ...
    def addRouterSwitchBridges(self,dict_rt,dict_sw):
        ...
        self.cursor.execute("SELECT child_ip,age,hard_address,interface,parent_ip FROM mib.topology_devices where age='-' and parent_type='Router'")
        routers = self.cursor.fetchall()
                    ...
                    if switch[0] == dict_sw[key]["ip"]:
                        for router in routers:
                            ...
                            if self.areInSameNetwork(router[0],switch[0],switch[1]) and router[1]=='-':
                                query = "select parent_ip,mac,port from (SELECT parent_ip,mac,port FROM mib.topology_hardware group by port having count(*)<=1) as dist where dist.mac='{}' and dist.parent_ip='{}'".format(router[2],switch[0])

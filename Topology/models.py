...
class TopologySsh(models.Model):
    ...
    def __str__(self):
        return "Hostname/IP : {}   ---  Port : {}  ---  Type : {}  ---  NetworkBits : {}".format(self.hostname,
                                                                                                 self.port, self.type,
                                                                                                 self.host_bits)
class TopologyServer(models.Model):
    id = models.AutoField(primary_key=True,null=False)
    ip = models.CharField(unique=True,max_length=100,null=False)
    description = models.CharField(max_length=300,null=True)
    class Meta:
        managed = True
        db_table = 'topology_server'
    def __str__(self):
        return "Server IP : {}   ---  Description : {} ".format(self.ip,self.description)

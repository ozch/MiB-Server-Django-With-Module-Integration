...
def PacketFlow(request):
    ...
    for flow in flows:
        ...
        if flow.in_bytes < 100 or packets == 0:
            continue
        net_flow.append(dict(start=start, packets=packets, dest_port=flow.l4_dst_port,
...
def FlowSharkAPI(request):
    ...
    flows = NetflowV.objects.filter(ts__gte=reference_time)
    for flow in flows:
        net_flow.append(dict(start=flow.ipv4_src_addr_a, end=flow.ipv4_dst_addr_a, reporter=flow.reporter_a,
                             src_port=flow.l4_src_port, dest_port=flow.l4_dst_port, pkts=flow.in_pkts,
                             bytes=flow.in_bytes))
    return JsonResponse(data=net_flow, safe=False)
